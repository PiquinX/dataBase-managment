...
export function AsideFilters () {
  ...
  return (
    <>
      <div onClick={handleClick} className='my-6 cursor-pointer w-max'><i className="text-blue-700 fa-solid fa-filter"></i> Filter</div>

      <aside className={`${cartContainerClass} flex justify-start fixed top-0 left-0 z-[500] backdrop-blur-sm h-full w-full bg-[#0006]`}>
        <div data-side-bar={`${sideBarData}`} className="order-2 z-[1000] flex-grow"></div>
