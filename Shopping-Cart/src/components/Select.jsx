...
export function Select ({ options, changeValue, value, id }) {
  ...
  return (
    <div className='relative'>
      <button id={id} data-dropdown-button onClick={clickHandle} className={`${buttonClass} w-[162px] duration-75 rounded border-2 flex py-1 pr-8 relative px-4 before:absolute before:duration-150 before:w-[.65rem] before:h-[.65rem] before:border-b-2 before:border-r-2 before:border-white before:right-3`}>{value}</button>
      <ul className={`${selectClass} absolute bg-[#202a37] shadow-select rounded w-[162px] `}>
        {
